...
if __name__ == "__main__":
    ...
    for key, data in articles.items():
        blog = base_html.replace("page_image", f"https://portal.kemu.site/assets/images/{key}.png").replace("page_title", f"{data['title']} | kemu tech blog").replace("page_description", data['body'])
    ...
    # ip address checker
    user_info_html = base_html.replace("page_image", "https://portal.kemu.site/assets/images/user-info.png").replace("page_title", "IP Address Checker | kemu tools").replace("page_description", "IP Addressなどの端末情報を確認できます。")
    user_info_html = user_info_html.replace("page_contents", """
    ...
    associate_html = base_html.replace("page_image", "https://portal.kemu.site/assets/images/associate.png").replace("page_title", "Amazon Associate Linker | kemu tools").replace("page_description", "アソシエイトIDからアソシエイトリンクを生成します。")
    associate_html = associate_html.replace("page_contents", """
    ...
    # JSON Typer
    typer_html = base_html.replace("page_image", "https://portal.kemu.site/assets/images/json-typer.png").replace("page_title", "JSON Typer | kemu tools").replace("page_description", "JSONの値をTypeScriptのinterface型に変換します。")
    typer_html = typer_html.replace("page_contents", """
        <h2>JSON Typer</h2>
    ...
    # realtime
    realtime_html = base_html.replace("page_image", "https://portal.kemu.site/assets/images/realtime.png").replace("page_title", "REALTIME | kemu tools").replace("page_description", "現在時刻を100ms単位で更新し、正確な日時をお知らせします。")
    realtime_html = realtime_html.replace("page_contents", """
        <h2>REALTIME</h2> 
    ...
    # JSON Formatter
    formatter_html = base_html.replace("page_image", "https://portal.kemu.site/assets/images/json-formatter.png").replace("page_title", "JSON Formatter | kemu tools").replace("page_description", "未整形のJSONデータを整形します。")
    formatter_html = formatter_html.replace("page_contents", """
    ...
    base64_html = base_html.replace("page_image", "https://portal.kemu.site/assets/images/base64.png").replace("page_title", "Base64 Encoder/Decoder | kemu tools").replace("page_description", "文字列データをBase64に、Base64データを文字列に変換します。")
    base64_html = base64_html.replace("page_contents", """
    ...
    # contact
    contact_html = base_html.replace("page_image", "https://portal.kemu.site/assets/images/contact.png").replace("page_title", "Contact | kemu portal").replace("page_description", "Contact")
    contact_html = contact_html.replace("page_contents", """
        <h2> Contact Me </h2>
    ...
    for key, data in logs.items():
        log = base_html.replace("page_image", data["photo"] or f"https://portal.kemu.site/assets/images/{key}.png").replace("page_title", f"{data['title']} | kemu logs").replace("page_description", data['body'])
